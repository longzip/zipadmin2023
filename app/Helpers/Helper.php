    ...
    */
    ...
    public static function getFormattedDateObject($date, $type = 'datetime', $array = true) {
        ...
        if ($type == 'datetime') {
            ...
            $dt['formatted'] = $tmp_date->format('dd-mm-yy H:i:s');
        } else {
            $dt['date'] = $tmp_date->format('Y-m-d');
            $dt['formatted'] = $tmp_date->format('dd-mm-yy');
        }
    ...
    public static function orderToXml($id){
        $order = Order::find($id);
        $str = View::make('orders.xml2')->with('order', $order)->render();
        $str = iconv(mb_detect_encoding($str, mb_detect_order(), true), "CP1258",$str);
        Storage::disk('dropbox')->put('SO' . $order->so_number .'.xml', $str);
        
        $str = View::make('glentries.xml')->with('order', $order)->render();
        $str = iconv(mb_detect_encoding($str, mb_detect_order(), true), "CP1258",$str);
        Storage::disk('dropbox')->put('Datcoc' . $order->so_number .'.xml', $str);

        $str = View::make('contacts.xml')->with('order', $order)->render();
        $str = iconv(mb_detect_encoding($str, mb_detect_order(), true), "CP1258",$str);
        Storage::disk('dropbox')->put('Contacts' . $order->so_number .'.xml', $str);
    }

    public static function saveQuoteLines(){
