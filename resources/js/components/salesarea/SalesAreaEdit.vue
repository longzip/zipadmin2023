    ...
    <div class="container-flush">
        <div class="card">
            ...
            <div class="card-footer">
                <button @click.prevent="updateSalesArea" class="btn btn-primary">Lưu</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created(){
        axios.get('/api/salesareas/' + this.salesAreaId)
        .then(res => this.form.fill(res.data.data))
        .catch(()=> location.replace("/khu-vuc"))
    },
    methods: {
        updateSalesArea() {
            this.form.put('/api/salesareas/' + this.salesAreaId)
                .then(() => {
