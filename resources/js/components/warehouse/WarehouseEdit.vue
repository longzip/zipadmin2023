    ...
    <div class="container-flush">
        <div class="card">
            ...
            <div class="card-footer">
                <button @click.prevent="updateWarehouse" class="btn btn-primary">Lưu</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created(){
        axios.get('/api/warehouses/' + this.warehouseId)
        .then(res => this.form.fill(res.data.data))
        .catch(()=> location.replace("/kho-khu-vuc"))
    },
    methods: {
        updateWarehouse() {
            this.form.put('/api/warehouses/' + this.warehouseId)
                .then(() => {
