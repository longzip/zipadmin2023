            ...
            <div class="col-md-3">
                ...
                <div class="card card-primary card-outline">
                    <div class="card-body box-profile">
                        <a class="" href="#" >
                            <h3 class="profile-username text-center">{{ contact.company.name_one}} <i v-show="contact.isPublished" class="fa fa-check"></i></h3>
                        </a>
                        <ul class="list-group list-group-unbordered mb-3">
                            <li class="list-group-item">
                                <b>Phone</b> <a class="float-right">{{ contact.company.phone_one}}</a>
                            </li>
                            <li class="list-group-item">
                                <b>Email</b> <a class="float-right">{{ contact.company.email_one}}</a>
                            </li>
                        </ul>
                        <router-link class="btn btn-primary btn-block" :to="{ name: 'editProject', params: { id: contact.id }}">Sửa thông tin</router-link>
                    </div>
                    <!-- /.card-body -->
                </div>

                <div class="card card-primary card-outline" v-if="contact.company.name_two != null">
                    <div class="card-body box-profile">
                        <a class="" href="#" >
                            <h3 class="profile-username text-center">{{ contact.company.name_two}} <i v-show="contact.isPublished" class="fa fa-check"></i></h3>
                        </a>
                        <ul class="list-group list-group-unbordered mb-3">
                            <li class="list-group-item">
                                <b>Phone</b> <a class="float-right">{{ contact.company.phone_two}}</a>
                            </li>
                            <li class="list-group-item">
                                <b>Email</b> <a class="float-right">{{ contact.company.email_two}}</a>
                            </li>
                        </ul>
                        <router-link class="btn btn-primary btn-block" :to="{ name: 'editProject', params: { id: contact.id }}">Sửa thông tin</router-link>
                    </div>
                        ...
                        <div class="card card-primary">
                            ...
                            <div class="card-body">
                                ...
                                <blockquote class="blockquote">
                                    <p class="mb-0">Dự Án: <i>{{contact.name_project}}</i></p>
                                    <p class="mb-0">Công Ty: <i>{{contact.company.name_company}}</i></p>
                                    <p class="mb-0">Chú Thích: <i>{{contact.note}}</i></p>
                                    <p class="mb-0"><small class="badge badge-info">{{ contact.status | ContactStatus}}</small></p>
                                    <footer class="blockquote-footer">Vào showroom {{getName(contact.coscenters)}} từ {{contact.start_time}} đến {{contact.end_time}} <cite title="Ngày">{{contact.start_date}}</cite> bởi {{contact.username}} - {{contact.user_id}}</footer>
                                </blockquote>
                            ...
                            <div class="card-footer clearfix">
                                <a v-show="!contact.isPublished" href="#" @click="publish(contact)" class="btn btn-sm btn-info float-left">Đạt KHDA</a>
                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <div class="card-tools">
                                    <router-link :to="{ name: 'taobaogiaDA', params: { contactId: this.contact.id }}" class="btn btn-primary"><i class="fas fa-plus-square"></i> Tạo báo giá</router-link>
                                </div>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
...
<script>
import QuoteList from '../elements/QuoteListDA.vue';
...
export default {
    ...
    methods: {
        ...
        storeContactComment(comment) {
            axios.put('api/project/' + this.contact.id + '/comment', {
                    body: comment
                })
                .then(response => {
        ...
        addKh15() {
            axios.put('api/project/' + this.contact.id + '/kh15s', {
                    name: this.kh15.name,
        ...
        addLost() {
            axios.put('api/project/' + this.contact.id + '/losts', {
                    name: this.lost.name,
        ...
        taokehoach() {
            axios.put('api/project/' + this.contact.id + '/tasks', {
        ...
        uploadImage(file) {
            ...
            axios.post('api/project/' + this.contact.id + '/image',
                    formData, {
                        headers: {
        ...
        //đang check
        themvideo(video_src) {
            axios.put('api/project/' + this.contact.id + '/videos', {
                    title: video_src,
                })
                .then(response => {
        ...
        addMedia() {
            ...
            axios.post('api/project/' + this.contact.id + '/attachmens',
                formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
